...
describe('Phase 2B beta status model', () => {
  ...
  it('keeps accepted parity gaps visible as pending or partial', () => {
    ...
    expect(status.parityItems).toEqual(
      expect.arrayContaining([
        ...
        expect.objectContaining({ label: 'SDR parity', state: 'not-fully-verified' }),
        expect.objectContaining({ label: 'Business Performance parity', state: 'not-fully-verified' }),
        expect.objectContaining({ label: 'Refund/ad-spend parity', state: 'not-fully-verified' }),
        expect.objectContaining({ label: 'Historical role parity', state: 'not-fully-verified' }),
