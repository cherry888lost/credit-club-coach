...
export interface CandidatePattern {
  ...
  source_close_type: string | null;
  
  // Per extraction-rules.md requirements
  candidate_category: CandidateCategory;
  candidate_title: string;
  confidence_score: number; // 0-100
  exact_prospect_quote: string;
  exact_rep_quote: string;
  why_this_mattered: string;
  why_this_is_repeatable: string;
  source_transcript_section: string;
  relevant_category_score: number;
  reason_for_queueing: string;
  suggested_action: 'reject' | 'approve' | 'promote';
}

// Per extraction-rules.md: Only these categories allowed
...
function mapToCandidateCategory(scoringCategory: string): CandidateCategory | null {
  const map: Record<string, CandidateCategory> = {
...
export function extractCandidatePatterns(
  ...
  transcript?: string // Optional: for extracting exact quotes
): CandidatePattern[] {
  ...
  for (const [catName, catData] of Object.entries(score.categories)) {
    ...
    let suggestedAction: 'reject' | 'approve' | 'promote' = 'approve';
  ...
  for (const objection of score.objections_handled_well || []) {
    ...
    if (!objection || objection.length < 10) continue;
    if (objection.length > 200) continue; // Too long to be a specific objection phrase

    // Check if this was actually detected as an objection
    const wasDetected = (score.objections_detected || []).some(o => 
      objection.toLowerCase().includes(o.toLowerCase()) || 
...
export async function saveCandidatePatterns(
...
export async function promoteToBenchmark(
  ...
  benchmarkData: {
    ...
    strongest_moments?: any[];
    objection_examples?: any[];
    key_lines_to_model?: any[];
    tags?: string[];
  }
): Promise<{ success: boolean; benchmark_id?: string; error?: string }> {
  // Get the pattern
 ...
 */
export async function updateObjectionFromPattern(
  supabase: SupabaseClient,
  patternId: string,
  orgId: string,
  objectionData: {
