  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard title="Total Calls" value={callsCount?.toString() || "0"} description="All time calls analyzed" />
        <StatCard title="Avg Score" value={avgScore} description="Average call quality score" />
        <StatCard title="Team Members" value={repsCount?.toString() || "0"} description="Active reps" />
        <StatCard title="Flagged Calls" value={flaggedCount?.toString() || "0"} description="Calls needing review" />
      </div>

      <div className="bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-800">
        <div className="p-6 border-b border-zinc-200 dark:border-zinc-800 flex items-center justify-between">
          <h2 className="text-lg font-medium text-zinc-900 dark:text-white">Recent Calls</h2>
          <Link href="/dashboard/calls" className="text-sm text-indigo-600 dark:text-indigo-400 hover:underline">View all</Link>
        </div>
        ...
        {recentCalls && recentCalls.length > 0 ? (
          ...
          <div className="p-12 text-center">
            <p className="text-zinc-500 dark:text-zinc-400">No calls recorded yet. Connect Fathom to start analyzing calls.</p>
            <Link href="/dashboard/settings" className="mt-4 inline-block px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg text-sm font-medium transition-colors">Connect Fathom</Link>
          </div>
        )}
      </div>
    </div>
